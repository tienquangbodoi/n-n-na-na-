...
async def custom_rate_limit_exceeded_handler(request: Request, exc: RateLimitExceeded):
    return JSONResponse(
        status_code=429,
        content={"detail": "Đạo hữu đã hỏi quá giới hạn 10 lần/phút. Tiên Trí cần nghỉ ngơi, vui lòng thử lại sau chốc lát!"}
    )
...
def get_gemini_models_list(vision=False):
    """Trả về danh sách mô hình Gemini ổn định, ưu tiên các model hoạt động tốt nhất"""
    return [
        "gemini-3.6-flash",
        "gemini-3.5-flash",
        "gemini-flash-latest",
...
def _call_gemini_sync(contents, vision=False, system_instruction=None):
    """Tự động thử lần lượt các model Gemini với Timeout 65 giây mỗi lượt"""
    ...
    for model_name in models:
        try:
            ...
            response = model.generate_content(contents, request_options={"timeout": 65})
            t1 = time.time()
...
async def generate_gemini_content_async(contents, vision=False, system_instruction=None):
    """Gọi Gemini API bất đồng bộ qua threadpool để không block event loop của FastAPI"""
    from starlette.concurrency import run_in_threadpool
    return await run_in_threadpool(_call_gemini_sync, contents, vision, system_instruction)


def generate_gemini_content(contents, vision=False, system_instruction=None):
    return _call_gemini_sync(contents, vision, system_instruction)
...
def retrieve_user_financial_context(conn, user_id: int, query_text: str = "") -> str:
    """Truy xuất toàn diện dữ liệu tài chính của người dùng (RAG Context) để cung cấp cho Gemini"""
    import re
    today = datetime.date.today()
    current_month = today.strftime('%Y-%m')
    first_day_cur = today.replace(day=1)
    prev_month = (first_day_cur - datetime.timedelta(days=1)).strftime('%Y-%m')
    
    # 1. Thông tin Đạo Hữu
    user = conn.execute('SELECT full_name, email FROM users WHERE id = ?', (user_id,)).fetchone()
    user_name = user['full_name'] if user else 'Đạo hữu'
    
    # 2. Túi Càn Khôn (Danh sách ví & Tổng số dư)
    wallets = conn.execute('SELECT wallet_name, balance, wallet_type FROM wallets WHERE user_id = ?', (user_id,)).fetchall()
    total_balance = sum(w['balance'] for w in wallets)
    wallet_lines = [f"  - {w['wallet_name']} ({w['wallet_type']}): {w['balance']:,.0f} VNĐ" for w in wallets]
    
    # 3. Dòng tiền tháng hiện tại và tháng trước
    cur_flow = conn.execute('''
        SELECT
    ...
    # 4. Top danh mục chi tiêu nhiều nhất
    top_cats = conn.execute('''
        SELECT c.category_name, SUM(t.amount) as total_spent, COUNT(t.id) as tx_count
        FROM transactions t
        JOIN categories c ON t.category_id = c.id
        WHERE t.user_id = ? AND t.transaction_type = 'EXPENSE'
        GROUP BY c.id ORDER BY total_spent DESC LIMIT 7
    ''', (user_id,)).fetchall()
    cat_lines = [f"  - {c['category_name']}: {c['total_spent']:,.0f} VNĐ ({c['tx_count']} giao dịch)" for c in top_cats]

    # 5. Hạn mức ngân sách tháng này
    budgets = conn.execute('''
        SELECT b.limit_amount, c.category_name,
               COALESCE((SELECT SUM(amount) FROM transactions WHERE user_id = ? AND category_id = b.category_id AND transaction_type = 'EXPENSE' AND strftime('%Y-%m', transaction_date) = b.month_year), 0) as spent
        FROM budgets b
    ...
    for b in budgets:
        pct = (b['spent'] / b['limit_amount'] * 100) if b['limit_amount'] > 0 else 0
        status = "🔥 Đã vượt hạn mức" if pct >= 100 else ("⚠️ Cảnh báo (>80%)" if pct >= 80 else "🟢 An toàn")
        budget_lines.append(f"  - {b['category_name']}: Đã tiêu {b['spent']:,.0f} / {b['limit_amount']:,.0f} VNĐ ({pct:.1f}%) -> {status}")

    # 6. Mục tiêu tiết kiệm (Tụ Khí)
    goals = conn.execute('''
        SELECT target_name, target_amount, current_amount, target_date, is_completed
        FROM saving_goals WHERE user_id = ? ORDER BY is_completed ASC, id DESC LIMIT 5
    ''', (user_id,)).fetchall()
    goal_lines = []
    for g in goals:
        pct = (g['current_amount'] / g['target_amount'] * 100) if g['target_amount'] > 0 else 0
        st = "Đã hoàn thành" if g['is_completed'] else f"Đang tích lũy ({pct:.1f}%)"
        goal_lines.append(f"  - {g['target_name']}: {g['current_amount']:,.0f} / {g['target_amount']:,.0f} VNĐ | Hạn: {g['target_date'] or 'Không đặt'} | {st}")

    # 7. Sổ nợ (Vay / Cho vay)
    debts = conn.execute('''
        SELECT debt_type, person_name, amount, due_date, is_settled, note
        FROM debts WHERE user_id = ? AND is_settled = 0 ORDER BY id DESC LIMIT 6
    ''', (user_id,)).fetchall()
    debt_lines = []
    for d in debts:
        d_type = "Tôi đang cho vay (Cần thu hồi)" if d['debt_type'] == 'LEND' else "Tôi đang nợ (Cần hoàn trả)"
        debt_lines.append(f"  - {d_type} {d['person_name']}: {d['amount']:,.0f} VNĐ | Hạn: {d['due_date'] or 'Không đặt'} | Ghi chú: {d['note'] or 'Không'}")

    # 8. Tìm kiếm giao dịch liên quan trực tiếp tới câu hỏi của người dùng (Targeted Search)
    search_lines = []
    if query_text:
        words = re.findall(r'[\w\d]+', query_text.lower())
        stop_words = {'tôi', 'cho', 'hỏi', 'thế', 'nào', 'là', 'gì', 'có', 'không', 'bao', 'nhiêu', 'tháng', 'này', 'ạ', 'ơi', 'ad', 'admin', 'ai', 'bot', 'gemini', 'tiên', 'trí', 'hãy', 'xem', 'xét', 'được', 'vào', 'ở', 'và', 'của'}
        keywords = [w for w in words if len(w) >= 2 and w not in stop_words]
        if keywords:
            clauses = " OR ".join(["t.note LIKE ? OR c.category_name LIKE ? OR w.wallet_name LIKE ?"] * len(keywords))
            params = [user_id]
            for kw in keywords:
                pat = f"%{kw}%"
                params.extend([pat, pat, pat])
            sql = f'''
                SELECT t.transaction_date, t.transaction_type, t.amount, t.note, c.category_name, w.wallet_name
                FROM transactions t
                LEFT JOIN categories c ON t.category_id = c.id
                LEFT JOIN wallets w ON t.wallet_id = w.id
                WHERE t.user_id = ? AND ({clauses})
                ORDER BY t.transaction_date DESC LIMIT 10
            '''
            matches = conn.execute(sql, params).fetchall()
            for m in matches:
                search_lines.append(f"  - [{m['transaction_date']}] {m['transaction_type']}: {m['amount']:,.0f} VNĐ | Danh mục: {m['category_name']} | Ví: {m['wallet_name']} | Ghi chú: {m['note'] or 'Không'}")

    # 9. 15 Giao dịch mới nhất
    recent_txs = conn.execute('''
        ...
        LEFT JOIN wallets w ON t.wallet_id = w.id
        WHERE t.user_id = ?
        ORDER BY t.transaction_date DESC, t.id DESC LIMIT 15
    ''', (user_id,)).fetchall()
    tx_lines = [f"  - [{t['transaction_date']}] {t['transaction_type']}: {t['amount']:,.0f} VNĐ | Danh mục: {t['category_name']} | Ví: {t['wallet_name']} | Ghi chú: {t['note'] or 'Không'}" for t in recent_txs]

    rag_text = f"""### 📊 BẢN ĐỒ TÀI CHÍNH THỰC TẾ CỦA ĐẠO HỮU ({user_name})
- **Thời gian hiện tại**: Ngày {today.strftime('%d/%m/%Y')} (Tháng {current_month})
- **Tổng tài sản ròng (Túi Càn Khôn)**: {total_balance:,.0f} VNĐ
- **Danh sách ví hiện có**:
{chr(10).join(wallet_lines) if wallet_lines else "  (Chưa có ví nào)"}

- **Dòng tiền Tháng {current_month} (Hiện tại)**:
  + Thu nhập: {cur_flow['income']:,.0f} VNĐ
  + Tiêu hao: {cur_flow['expense']:,.0f} VNĐ
  + Thặng dư/Tiết kiệm: {cur_flow['income'] - cur_flow['expense']:,.0f} VNĐ
- **Dòng tiền Tháng {prev_month} (Tháng trước)**:
  + Thu nhập: {prev_flow['income']:,.0f} VNĐ
  + Tiêu hao: {prev_flow['expense']:,.0f} VNĐ
  + Thặng dư: {prev_flow['income'] - prev_flow['expense']:,.0f} VNĐ

- **Top danh mục chi tiêu nhiều nhất**:
{chr(10).join(cat_lines) if cat_lines else "  (Chưa có giao dịch chi tiêu)"}

- **Hạn mức ngân sách tháng {current_month}**:
{chr(10).join(budget_lines) if budget_lines else "  (Chưa thiết lập hạn mức tháng này)"}

- **Mục tiêu tiết kiệm (Tụ Khí)**:
{chr(10).join(goal_lines) if goal_lines else "  (Chưa có mục tiêu)"}

- **Sổ nợ (Vay / Cho vay)**:
{chr(10).join(debt_lines) if debt_lines else "  (Không có khoản nợ nào chưa giải quyết)"}
"""
    if search_lines:
        rag_text += f"\n- **Giao dịch liên quan đến câu hỏi ('{query_text}')**:\n" + "\n".join(search_lines) + "\n"

    rag_text += f"\n- **15 giao dịch gần đây nhất**:\n" + ("\n".join(tx_lines) if tx_lines else "  (Chưa có giao dịch nào)")
    return rag_text


@app.post("/api/ai/chat")
async def ai_chat(body: ChatBody, user: dict = Depends(get_current_user)):
    """Khí Linh Tiên Trí — siêu trợ lý AI Gemini kết hợp RAG thông minh (bản async không block event loop)"""
    t_start = time.time()
    user_id = user["user_id"]
    
    with get_db() as conn:
        t_db_0 = time.time()
        rag_context = retrieve_user_financial_context(conn, user_id, body.message)

        # Lấy 5 lượt hội thoại gần nhất để duy trì ngữ cảnh
    ...
    if history_rows:
        ...
        for r in history_items:
            ...
            lines.append(f"Tiên Trí: {r['ai_response'][:250]}...")
        recent_history = "Lịch sử đàm đạo gần đây:\n" + "\n".join(lines) + "\n\n"

    system_instruction = """Bạn là "Khí Linh Tiên Trí" — siêu trí tuệ AI thông thái vận hành bởi Google Gemini trong hệ thống quản lý tài chính Càn Khôn Linh Thạch Các.
Bạn vừa là một Đại Tông Sư Cố Vấn Tài Chính sắc bén, am hiểu tường tận mọi ngóc ngách dữ liệu của ký chủ/đạo hữu, vừa là một mô hình Large Language Model (Gemini) uyên bác, thông tuệ toàn diện về mọi lĩnh vực tri thức nhân gian.

Nguyên Tắc Ứng Xử & Trả Lời:
1. VỀ TÀI CHÍNH & DỮ LIỆU CỦA ĐẠO HỮU (RAG):
   - Luôn tham chiếu và phân tích sâu sát dữ liệu thực tế được cung cấp trong mục [BẢN ĐỒ TÀI CHÍNH THỰC TẾ].
   - Khi đạo hữu hỏi về số dư, chi tiêu, thu nhập, danh mục, ví tiền, hạn mức ngân sách, khoản nợ, mục tiêu tiết kiệm hay tìm kiếm giao dịch: Hãy nêu chính xác con số (VNĐ), ngày tháng, danh mục, ví, ghi chú cụ thể. Không được nói chung chung, không được đoán mò khi đã có dữ liệu.
   - Nếu dữ liệu tháng hiện tại còn ít hoặc bằng 0, hãy chỉ rõ điều đó và đối chiếu thêm với tháng trước hoặc các giao dịch gần đây để đưa ra nhận định khách quan, sâu sắc.
   - Chủ động đưa ra lời khuyên tài chính thông minh, chiến lược chi tiêu, cảnh báo nguy cơ vượt hạn mức (Tẩu Hỏa Nhập Ma) hoặc gợi ý cách tối ưu dòng tiền.

2. VỀ KIẾN THỨC TỔNG QUÁT & GIAO TIẾP ĐA NĂNG (GEMINI LLM):
   - Bạn sở hữu trọn vẹn trí tuệ của Google Gemini: giải thích khoa học, viết code/lập trình, giải toán, tư vấn đời sống, sáng tác thơ văn, dịch thuật, phân tích logic, triết học, tâm sự...
   - KHÔNG BAO GIỜ từ chối các câu hỏi ngoài tài chính. KHÔNG nói "ta chỉ là trợ lý tài chính nên không biết". Hãy trả lời uyên bác, sâu sắc, hữu ích và tự nhiên như chính Gemini.

3. PHONG THÁI & NGÔN NGỮ:
   - Xưng hô lịch thiệp: Tự xưng là "Khí Linh" / "Tiên Trí", gọi người dùng là "Đạo hữu" hoặc "Ký Chủ".
   - Mang phong vị Tiên Hiệp huyền ảo nhẹ nhàng, trang nhã nhưng văn phong phải hiện đại, gãy gọn, mạch lạc, dễ đọc. Nếu người dùng hỏi bằng tiếng Anh hoặc yêu cầu văn phong hiện đại/kỹ thuật, hãy linh hoạt đáp ứng hoàn hảo.
   - Định dạng văn bản bằng Markdown rõ ràng: dùng gạch đầu dòng, in đậm các con số quan trọng, bảng so sánh hoặc khối code khi cần thiết.
"""

    prompt = f"""[BẢN ĐỒ TÀI CHÍNH THỰC TẾ CỦA ĐẠO HỮU]:
{rag_context}
...
{recent_history}Câu hỏi mới của Đạo hữu: {body.message}"""
    ...
    try:
        t_ai_0 = time.time()
        ai_answer = await generate_gemini_content_async(prompt, vision=False, system_instruction=system_instruction)
        t_ai_1 = time.time()
        ...
        with get_db() as conn:
            conn.execute(
                ...
                (user_id, body.message, ai_answer)
            )

        t_end = time.time()
        print(f"[AI Chat RAG Metric] DB: {t_db_1 - t_db_0:.3f}s | Gemini API: {t_ai_1 - t_ai_0:.3f}s | Total: {t_end - t_start:.3f}s")
