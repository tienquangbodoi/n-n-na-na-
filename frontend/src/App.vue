  ...
  setup() {
    ...
    function formatChatText(text) {
      ...
      let s = text
        ...
        .replace(/</g, '&lt;')
        .replace(/>/g, '&gt;')
      s = s.replace(/```([\s\S]*?)```/g, '<pre style="background:rgba(0,0,0,0.5);padding:8px;border-radius:6px;overflow-x:auto;margin:6px 0;font-family:monospace;"><code>$1</code></pre>')
      s = s.replace(/`([^`]+)`/g, '<code style="background:rgba(255,255,255,0.1);padding:2px 5px;border-radius:4px;font-family:monospace;">$1</code>')
      s = s.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>')
      s = s.replace(/\*(.*?)\*/g, '<em>$1</em>')
      s = s.replace(/^[ \t]*[-*][ \t]+(.*)$/gm, '<li style="margin-left:18px;list-style-type:disc;">$1</li>')
      s = s.replace(/\n/g, '<br/>')
      return s
    }
    ...
    async function sendChat() {
      ...
      try {
        ...
        if (!response.ok) {
           let errorDetail = "Không thể kết nối với thần trí AI."
           try {
               const errData = await response.json()
               if (errData.detail) errorDetail = errData.detail
           } catch (e) {
               if (response.status === 429) errorDetail = "Đạo hữu đã hỏi quá giới hạn 10 lần/phút. Tiên Trí cần nghỉ ngơi, vui lòng thử lại sau chốc lát!"
           }
        ...
        while (true) {
          ...
          chatMessages.value[aiMsgIndex].text += chunk
          scrollChat()
        }
      } catch (err) {
        if (!Array.isArray(chatMessages.value)) chatMessages.value = []
